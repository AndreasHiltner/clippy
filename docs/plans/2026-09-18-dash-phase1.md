# Dash Phase 1 (The Brain) — Implementation Plan

> **For Hermes:** Use subagent-driven-development skill to implement this plan task-by-task.

**Goal:** Dash as a Hermes plugin — proactive error observer with a mood system + `/dash` helpdesk.

**Architecture:** One plugin package `~/.hermes/plugins/dash/` with `register(ctx)`. The `post_tool_call` hook classifies tool errors deterministically and delivers **template-based wording synchronously** via `ctx.inject_message` — no LLM in the agent loop, ever. The `/dash` command answers from the knowledge base first, then falls back to a **synchronous** `ctx.llm.complete()` call. Mood + memory + throttle are pure functions over `ctx.state`.
...
name: dash
version: 0.1.0
description: "Dash — the proactive assistant: detects repeated tool errors and helps with Hermes questions. Mood system, memory, throttle."
author: andreas
kind: standalone
...
def register(ctx) -> None:
    """Registers the /dash command + post_tool_call/on_session_start hooks."""
    pass
```
...
```bash
python3 -c "import dash; dash.register(type('C',(),{'register_command':lambda *a,**k:None,'register_hook':lambda *a,**k:None})())"
...
git commit -m "chore: dash plugin skeleton + manifest"
```
...
**Objective:** Curated Markdown knowledge base for the `/dash` helpdesk.
...
```bash
git add knowledge/
git commit -m "docs: dash knowledge base"
```
...
## Task 6: `/dash` Helpdesk Command

**Depends on:** Task 2, Task 4, Task 5
**Parallel with:** —
**Blocks:** Task 9

**Objective:** `register_command("dash", ...)` — knowledge base first, then synchronous LLM fallback.
...
SYSTEM_PROMPT = (
    "You are Dash, the pencil-shaped helper. Answer briefly, with charm, "
...
python3 - <<'PY'
import dash
dash._CTX = type('C', (), {'llm': type('L', (), {'complete': lambda self, m, **k: (_ for _ in ()).throw(RuntimeError('no LLM in this path'))})()})()
print(dash._dash_command("gateway won't start")[:60])
PY
...
```bash
git add __init__.py
git commit -m "feat: /dash helpdesk command (knowledge first, sync LLM fallback)"
```
...
```python
# tests/test_hook.py
from dash import __init__ as dash_mod
...
4. **`ctx.llm.complete()` (sync)** is the correct call from the `/dash` command handler; it returns `PluginLlmCompleteResult` with `.text`. Trust gates are fail-closed — without `plugins.entries.dash.llm.*` config only the default path runs (no model override), correct for Phase 1.
